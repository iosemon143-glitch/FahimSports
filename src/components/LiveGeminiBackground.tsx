...
export const LiveGeminiBackground = React.memo<LiveGeminiBackgroundProps>(({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const dpr = Math.min(window.devicePixelRatio || 1, 2); // Cap at 2 for silky performance
      width = rect.width;
      height = rect.height;
  ...
  return (
    ...
    >
      ...
      <canvas
        ref={canvasRef}
        className="absolute inset-0 w-full h-full block z-0 filter blur-[12px] sm:blur-[16px]"
