...
import { GeminiTheme, GeminiPreset } from './components/LiveGeminiBackground';
import { ColorGradingModal } from './components/ColorGradingModal';

const WISHLIST_STORAGE_KEY = 'next_level_wishlist';
const WISHLIST_MIGRATION_KEY = 'next_level_wishlist_v2';
...
export function App() {
  ...
  const [wishlist, setWishlist] = useState<string[]>(() => {
    try {
      ...
      // Remove the old hard-coded default jersey exactly once after upgrading.
      if (!localStorage.getItem(WISHLIST_MIGRATION_KEY)) {
        localStorage.setItem(WISHLIST_MIGRATION_KEY, '1');
        return cleaned.filter((id) => id !== 'prod-jersey');
      }
      return cleaned;
    } catch {
      return [];
  ...
  const [geminiSpeed, setGeminiSpeed] = useState<'calm' | 'normal' | 'dynamic'>('calm');
    ...
    try {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed)) {
          // Remove the old fabricated demo orders from previous app versions.
          return parsed.filter((order) => !['SP-99201', 'SP-84102'].includes(order?.id));
        }
      }
    } catch {
      // fallback
    ...
    <div className="min-h-screen h-[100dvh] relative text-[#141416] flex flex-col items-center justify-center p-0 sm:p-4 overflow-hidden select-none font-sans">
      ...
      <header className="hidden sm:flex items-center justify-between w-full max-w-[420px] mb-3 px-2 text-white/80 text-xs relative z-30">
        <div className="flex items-center space-x-2">
          ...
          <span className="font-semibold tracking-wide text-white drop-shadow-xs">NEXT LEVEL</span>
        </div>
      ...
      >
        {/* Static premium atmosphere: no continuous canvas animation, keeping Android WebView smooth. */}
        <div
          aria-hidden="true"
          className={`nl-static-atmosphere ${geminiTheme === 'cosmic-dark' ? 'nl-static-dark' : 'nl-static-light'}`}
        />
